...
import br.dev.corsac.ledger.orcamento.mensal.api.dtos.OrcamentoMensalUpdateRequest
...
fun OrcamentoMensal.toStatement(stmt: UpdateBuilder<*>) {
    stmt[OrcamentosMensaisTable.usuarioId] =
        this.idUsuario
    stmt[OrcamentosMensaisTable.ano] =
        this.anoMes.ano.value
    stmt[OrcamentosMensaisTable.mes] =
        this.anoMes.mes.number
    stmt[OrcamentosMensaisTable.slug] = this.slug
    stmt[OrcamentosMensaisTable.dataInicio] =
        this.dataInicio
    stmt[OrcamentosMensaisTable.dataFim] =
        this.dataFim
    stmt[OrcamentosMensaisTable.seqReceita] =
        this.seqReceita
    stmt[OrcamentosMensaisTable.seqDespesa] =
        this.seqDespesa
}

fun OrcamentoMensalUpdateRequest.toStatement(stmt: UpdateBuilder<*>) {
    this.ano?.let {
        stmt[OrcamentosMensaisTable.ano] = it
    }
    this.mes?.let {
        stmt[OrcamentosMensaisTable.mes] = it
    }
    ...
    this.dataFim?.let {
        stmt[OrcamentosMensaisTable.dataFim] = it
    }
}
